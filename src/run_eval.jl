...
function run_eval(
	...
	csv_cols = string(
		...
		"perc_delete,", # percentage of unknown factors
		"p_cohorts,", # probability for the first cohort (remaining cohorts are uniform)
	...
	open(logfile, "a") do io
		...
		for (root, dirs, files) in walkdir(input_dir)
			for f in files
				...
				p_cohorts = parse(Float64, string(p[1], ".", p[2:end]))
				n_rvs = numrvs(fg)
				n_factors = numfactors(fg)
				n_max_args = maximum([length(rvs(f)) for f in factors(fg)])
				for p_delete in [0.05, 0.1, 0.15, 0.2]
					# Delete 5%-10% of factors, randomly chosen
					delete_potentials!(fg, ceil(Int, p_delete * n_factors))
